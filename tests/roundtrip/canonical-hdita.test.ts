/**
 * Unit tests for the canonical model ↔ HDITA adapter.
 *
...
// --- Regression: an unknown wrapper <div> must not delete its children ---
// (parseDiv returned null for an unrecognized div and the caller dropped it,
// contradicting the lossless-round-trip promise.)

describe("hditaToCanonical: unknown wrapper divs", () => {
  it("preserves children of an unrecognized <div> instead of dropping them", async () => {
    const html =
      '<div class="layout-wrapper"><p>kept content</p></div>';
    const canon = await hditaToCanonical(html);
    const back = canonicalToTiptap(canon);
    const text = JSON.stringify(back);
    expect(text).toContain("kept content");
  });

  it("preserves children nested several unknown divs deep", async () => {
    const html = "<div><div><p>deep</p></div></div>";
    const canon = await hditaToCanonical(html);
    expect(JSON.stringify(canonicalToTiptap(canon))).toContain("deep");
  });
});
