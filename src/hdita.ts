 ...
 * shape for Topicary content (CANONICAL_MODEL.md §4). It maps cleanly onto
 * ProseMirror's DOM model and is one DITA-OT (`org.lwdita`) hop from normalized
 * DITA, so it is the natural serialization target.
 *
 ...
 * canonical documents in normalized form — that round-trip is the headline
 * guarantee of the own-model-canonical architecture (your content is not
 * trapped in TipTap's shape).
 *
 * Design notes:
 * - HDITA here is intentionally **lossless** (full HTML5 tables with spans,
 *   code groups, `<hr>`, blockquotes are all preserved). The LwDITA downgrades
 *   noted in CANONICAL_MODEL.md §4 (simpletable drops spans, etc.) happen at
 *   the later DITA-OT hop, NOT in HDITA.
 ...
 * `cheerio` (an optional peer dependency) — matching how github-sync.ts treats
 * `octokit`.
 *
 * @module @topicary/export/hdita
 */
 ...
 * `canonicalToHdita`. Lazily loads `cheerio` (optional peer dependency).
 */
...
function parseDiv(el: DomNode): CanonicalNode | null {
  ...
  // Unknown div — descend so we don't lose its children.
  return null;
}
